...
from template.devtools.secret_scan import PATTERNS, _scan_text
...
def _run_secret_scan(repo: Path, scope: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "template.devtools.secret_scan", "--scope", scope],
        cwd=repo,
        check=False,
        capture_output=True,
        text=True,
...
def _init_repo(repo: Path) -> None:
    _git(repo, "init")
    _git(repo, "config", "user.name", "Template Tests")
    _git(repo, "config", "user.email", "template-tests@example.com")
